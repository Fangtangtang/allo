    ...
    def top():
        ...
        def VEC(
            ...
            for idx in allo.grid(INST_NUM, name="inst_outer"):
                ...
                # reduction
                if inst == VREDMAX or inst == VREDSUM:
                    # reduction max
                    # - 8 bits
                    base_8b: int8 = rs1[0:8]
                    ...
                    # reduction sum
                    redsum_8b: int8 = rs1[0:8]
                    redsum_16b: int16 = rs1[0:16]
                    redsum_32b: int32 = rs1
                    ...
                    if inst == VREDMAX:
                        if ele_type == EEW8:
                            vd_r[0:8] = redmax_8b
                        if ele_type == EEW16:
                            vd_r[0:16] = redmax_16b
                        if ele_type == EEW32:
                            vd_r[0:32] = redmax_32b
                    if inst == VREDSUM:
                        if ele_type == EEW8:
                            vd_r[0:8] = redsum_8b
                        if ele_type == EEW16:
                            vd_r[0:16] = redsum_16b
                        if ele_type == EEW32:
                            vd_r[0:32] = redsum_32b
                    vd_[idx] = vd_r
