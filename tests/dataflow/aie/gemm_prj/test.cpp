...
int main(int argc, const char *argv[]) {
  ...
  auto bo_in0 = xrt::bo(device, 131072 * sizeof(std::bfloat16_t),
  ...
  if (ifile0_size != 131072 * sizeof(std::bfloat16_t)) {
    std::cerr << "Error: Invalid input file, byte number mismatch.\n";
    return 1;
  }
  std::vector<std::bfloat16_t> vec0(131072);
  ifile0.read(reinterpret_cast<char*>(vec0.data()), 131072 * sizeof(std::bfloat16_t));
  srcVec0.insert(srcVec0.end(), vec0.begin(), vec0.end());
  memcpy(bufIn0, srcVec0.data(), (srcVec0.size() * sizeof(std::bfloat16_t)));

  auto bo_out1 = xrt::bo(device, 65536 * sizeof(std::bfloat16_t),
                        XRT_BO_FLAGS_HOST_ONLY, kernel.group_id(4));
  auto bo_in2 = xrt::bo(device, 131072 * sizeof(std::bfloat16_t),
  ...
  if (ifile1_size != 131072 * sizeof(std::bfloat16_t)) {
    std::cerr << "Error: Invalid input file, byte number mismatch.\n";
    return 1;
  }
  std::vector<std::bfloat16_t> vec1(131072);
  ifile1.read(reinterpret_cast<char*>(vec1.data()), 131072 * sizeof(std::bfloat16_t));
  ...
  ofile2.write(reinterpret_cast<const char*>(bufOut1 + 0), 65536 * sizeof(std::bfloat16_t));
  ofile2.close();
  ifile1.close();
  if (trace_size > 0) {
