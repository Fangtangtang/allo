...
int main(int argc, const char *argv[]) {
  ...
  auto bo_in0 = xrt::bo(device, 8192 * sizeof(std::bfloat16_t),
  ...
  if (ifile0_size != 4096 * sizeof(std::bfloat16_t)) {
    std::cerr << "Error: Invalid input file, byte number mismatch.\n";
    return 1;
  }
  std::vector<std::bfloat16_t> vec0(4096);
  ifile0.read(reinterpret_cast<char*>(vec0.data()), 4096 * sizeof(std::bfloat16_t));
  ...
  if (ifile3_size != 4096 * sizeof(std::bfloat16_t)) {
    std::cerr << "Error: Invalid input file, byte number mismatch.\n";
    return 1;
  }
  std::vector<std::bfloat16_t> vec3(4096);
  ifile3.read(reinterpret_cast<char*>(vec3.data()), 4096 * sizeof(std::bfloat16_t));
  srcVec0.insert(srcVec0.end(), vec3.begin(), vec3.end());
  ...
  auto bo_in2 = xrt::bo(device, 8192 * sizeof(std::bfloat16_t),
  ...
  if (ifile1_size != 4096 * sizeof(std::bfloat16_t)) {
    std::cerr << "Error: Invalid input file, byte number mismatch.\n";
    return 1;
  }
  std::vector<std::bfloat16_t> vec1(4096);
  ifile1.read(reinterpret_cast<char*>(vec1.data()), 4096 * sizeof(std::bfloat16_t));
  ...
  auto ifile4_size = ifile4.tellg();
  ifile4.seekg(0, std::ios::beg);
  if (ifile4_size != 4096 * sizeof(std::bfloat16_t)) {
    std::cerr << "Error: Invalid input file, byte number mismatch.\n";
    return 1;
  }
  std::vector<std::bfloat16_t> vec4(4096);
  ifile4.read(reinterpret_cast<char*>(vec4.data()), 4096 * sizeof(std::bfloat16_t));
  srcVec2.insert(srcVec2.end(), vec4.begin(), vec4.end());
  memcpy(bufIn2, srcVec2.data(), (srcVec2.size() * sizeof(std::bfloat16_t)));

  auto bo_out3 = xrt::bo(device, 262144 * sizeof(std::bfloat16_t),
  ...
  ofile2.write(reinterpret_cast<const char*>(bufOut1 + 0), 262144 * sizeof(std::bfloat16_t));
  ofile2.close();
  ifile1.close();
  ifile4.close();

  bo_out3.sync(XCL_BO_SYNC_BO_FROM_DEVICE);
  std::bfloat16_t *bufOut3 = bo_out3.map<std::bfloat16_t *>();
  std::ofstream ofile5("output5.data", std::ios::binary);
  ofile5.write(reinterpret_cast<const char*>(bufOut3 + 0), 262144 * sizeof(std::bfloat16_t));
  ofile5.close();
  if (trace_size > 0) {
