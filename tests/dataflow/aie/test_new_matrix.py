...
def test_matrix_scalar_add():
    ...
    def top(A: Ty[M, N], B: Ty[M, N]):
        ...
        def core(local_A: Ty[M, N] @ LyA, local_B: Ty[M, N] @ LyA):
            local_B.put(allo.add(local_A.get(), 1))
...
def test_matrix_matrix_add():
    ...
    def top(A: Ty[M, N], B: Ty[M, N], C: Ty[M, N]):
        ...
        def core(
            ...
            local_C.put(allo.add(local_A.get(), local_B.get()))
...
def test_gemm_1D_mixed():
    ...
    def top(A: TyI[M, K], B: TyI[K, N], C: TyO[M, N]):
        ...
        def gemm(
            ...
            local_C_part: TyO[M // P0, N] = allo.matmul(local_A.get(), local_B.get())
...
def test_gemm_atb():
    ...
    def top(A0: Ty[Ma, K], A1: Ty[Ma, K], B: Ty[K, N], C: Ty[M, N]):
        ...
        def gemm0(local_A: Ty[Ma, K]):
            c0 = allo.matmul(local_A.get(), pipeB[0].get())
            pipeC[0].put(c0)

        @df.kernel(mapping=[1], args=[A1])
        def gemm1(local_A: Ty[Ma, K]):
            c1 = allo.matmul(local_A.get(), pipeB[1].get())
            pipeC[1].put(c1)

        @df.kernel(mapping=[1], args=[C])
        def store(local_C: Ty[M, N]):
            local_C_: Ty[M, N]
            local_C_[:Ma, :] = pipeC[0].get()
            local_C_[Ma:, :] = pipeC[1].get()
            local_C.put(local_C_)
