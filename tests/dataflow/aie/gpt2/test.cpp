...
int main(int argc, const char *argv[]) {
  ...
  for (size_t i = 0; i < 100; i++) { // Load instruction sequence
    ...
    auto bo_in0 = xrt::bo(device, 262144 * sizeof(std::bfloat16_t),
    ...
    auto run =
        kernel(opcode, bo_instr, instr_v.size(), bo_in0, bo_out1, bo_trace);
    ...
    //       auto run = kernel(opcode, bo_instr, instr_v.size(), bo_in0,
    //       bo_out1, bo_trace); ert_cmd_state r = run.wait(); if (r !=
    ...
    //       start).count(); total_npu_time += npu_time; npu_time_min =
    //       (npu_time < npu_time_min) ? npu_time : npu_time_min;
    //   }
    //   std::cout << "Avg NPU execution time: " << total_npu_time /
    //   n_test_iterations << "us\n"; std::cout << "Min NPU execution time: " <<
    //   npu_time_min << "us\n";
    // }
    auto e2e_end = std::chrono::high_resolution_clock::now();
    float e2e_time = std::chrono::duration_cast<std::chrono::microseconds>(
                         e2e_end - e2e_start)
                         .count();
    std::cout << "E2E execution time: " << e2e_time << "us\n";
  ...
  // ofile1.write(reinterpret_cast<const char *>(bufOut1 + 0),
  //              262144 * sizeof(std::bfloat16_t));
  ofile1.close();
  // if (trace_size > 0) {
