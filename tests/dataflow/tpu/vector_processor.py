...
def vpu(
    Inst: uint8[INST_NUM],
    EleType: uint8[INST_NUM],
    VV: bool[INST_NUM],
    VS1: uint256[INST_NUM],
    VS2: uint256[INST_NUM],
    RS1: uint32[INST_NUM],
    VD: uint256[INST_NUM],
):
    @df.kernel(mapping=[1], args=[Inst, EleType, VV, VS1, VS2, RS1, VD])
    def VEC(
...
if __name__ == "__main__":
    ...
    mod = df.build(vpu, target="simulator")
