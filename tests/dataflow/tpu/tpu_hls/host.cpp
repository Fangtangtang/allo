...
int main(int argc, char** argv) {
    ...
    std::vector<uint8_t>  source_in0(20);
    ifile0.read(reinterpret_cast<char*>(source_in0.data()), 20);
    ...
    std::vector<uint8_t>  source_in1(1024);
    ifile1.read(reinterpret_cast<char*>(source_in1.data()), 1024);
    std::ifstream ifile2("input2.data", std::ios::binary);
    if (!ifile2.is_open()) {
      std::cerr << "Error: Could not open input file.\n";
      return 1;
    }
    ...
    std::vector<uint8_t>  source_in2(1024);
    ifile2.read(reinterpret_cast<char*>(source_in2.data()), 1024);
    ...
    OCL_CHECK(err, cl::Buffer buffer_in0(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, 20, source_in0.data(), &err));
    OCL_CHECK(err, cl::Buffer buffer_in1(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, 1024, source_in1.data(), &err));
    OCL_CHECK(err, cl::Buffer buffer_in2(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE, 1024, source_in2.data(), &err));
    ...
    ofile.write(reinterpret_cast<const char*>(source_in2.data()), 1024);
    ofile.close();
