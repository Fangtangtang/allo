...
void gelu_tanh(T *input_tensor, T *output_tensor) {
  ...
  for (int iter = 0; iter < BATCH_SIZE; iter++) {
    T *__restrict input_ptr = input_tensor;
    T *__restrict output_ptr = output_tensor;
    ...
    input_tensor += FEATURE_DIM;
    output_tensor += FEATURE_DIM;
  }
  event1();
}
...
void gelu(bfloat16 A_in[4][512], bfloat16 B_out[4][512]) {
  gelu_tanh<bfloat16, 4, 512>(&A_in[0][0], &B_out[0][0]);
}
