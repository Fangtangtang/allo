...
void layer_norm_single_batch_no_bias(T_in *input_tensor, T_in *weight, T_out *output_tensor) {
  ...
  for (int i = 0; i < F; i++){
    ...
    mean += aie::reduce_add(input_vec) / HIDDEN;
  }
  ...
  vec_t variance_vec = aie::broadcast<T_in, vec_factor>(variance_sum / HIDDEN + EPS);
...
template <typename T_in, typename T_out, const int SEQ_LEN, const int HIDDEN>
void rms_norm_single_batch(T_in *input_tensor, T_in *weight, T_out *output_tensor) {
  constexpr int vec_factor = 16;
  using vec_t = aie::vector<T_in, vec_factor>;
  event0();
  T_in *__restrict input_ptr = input_tensor;
  ...
  vec_t square_sum_vec = aie::broadcast<T_in, vec_factor>(square_sum/ HIDDEN + EPS);
...
void layer_norm(float *A_in, float *B_in, float *C_out) {
  layer_norm_single_batch_no_bias<float, float, 1, 1024>(A_in, B_in, C_out);
}
