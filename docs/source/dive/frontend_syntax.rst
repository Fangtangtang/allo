...
.. warning::
   ...
   ``break`` and ``continue`` statements are currently **not supported** in Allo loops.
...
.. important::
    ...
    The conditions used in ``meta_if``, ``meta_elif`` must be **compile-time constants**.
...
.. important::
    ...
    The loop bounds and step values in ``meta_for`` must be **compile-time constants**.
