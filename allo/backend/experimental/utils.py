        ...
        for op in operations:
            ...
            # matmul
                ...
                if (
                    ...
                    output_idx.append(2)
                    call_builtin = True
                    kernel_name = f"matmul_scalar_{dtype_1}_{out_dtype}"
                    operands = [
                        op.inputs[0],
                        op.inputs[1],
                        op.outputs[0],
                    ...
                    output_idx.append(2)
                    call_builtin = True
                    kernel_name = f"matmul_scalar_{dtype_1}x{dtype_2}_{out_dtype}"
                    operands = [
                        op.inputs[0],
                        op.inputs[1],
                        op.outputs[0],
...
"""
    ...
    for src in include_src:
        ...
        elif "mm_mixed.cc" in src:
            with open(
                os.path.expandvars(f"$ALLO_EXTERNAL_KERNEL_DIR/mm_mixed.cc"),
                "r",
                encoding="utf-8",
            ) as f:
                mm_kernel = f.read()
...
context_creation = """
  ...
  float c_time = std::chrono::duration_cast<std::chrono::microseconds>(end_create - start_create).count();
  std::cout << "Context creation time: " << c_time << "us\\n";
  ...
  float i_time = std::chrono::duration_cast<std::chrono::microseconds>(end_inst - start_inst).count();
  std::cout << "Inst time: " << i_time << "us\\n";
...
def codegen_profile_host(
    global_tensors: dict[int, DTensor], runtime_args: list[RuntimeArgs]
):
    """
    Generate the C++ code for external kernels for host CPU.
    """
    code = profile_host_header
    code_seg = context_creation
    with format_code(indent=2):
        ...
        code_seg += format_str("char *bufTrace = bo_trace.map<char *>();")
        code_seg += format_str("if (trace_size > 0)")
        code_seg += format_str("  memset(bufTrace, 0, trace_size);", strip=False)
        ...
        for idx, arg in enumerate(runtime_args):
            if len(arg.global_tensors) == 0:
                continue
            if arg.is_input:
                code_seg += format_str(f"bo_in{idx}.sync(XCL_BO_SYNC_BO_TO_DEVICE);")
        code_seg += format_str("if (trace_size > 0)")
        code_seg += format_str(
            "  bo_trace.sync(XCL_BO_SYNC_BO_TO_DEVICE);", strip=False
        )
        code_seg += format_str(
