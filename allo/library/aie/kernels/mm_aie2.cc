 ...
 *   https://github.com/Xilinx/mlir-aie/blob/91692b3546f1ad93a46c7228647f443f19dc58be/aie_kernels/aie2/mm.cc
 *
 * Acknowledgements:
 ...
 * statements, as well as project-specific adjustments for Allo. This variant
 * additionally adds column-major B/C operand support (the b_row_maj/c_row_maj
 * template parameters and aie::transpose paths) and uses the portable AIE_*
 * loop pragmas from aie_kernel_utils.h.
 */
...
// The Allo AIE backend injects the `#include` preamble and the `extern "C"`
// block of instantiations (one `*_c_func(...)` invocation per dtype/dimension
// combination) when it lowers a design onto this kernel; the file itself only
// defines the macros and the templates those invocations expand into.
...
// `matmul_scalar` has no per-precision wrapper template, so the column-major
// flags are threaded into the template instantiation directly here.
